...
export default function Dashboard() {
  ...
  const [data, setData] = useState({}); //this data will be passed to Calendar
  const now = new Date();
  ...
  const statuses = {
    // num_days: 14, // we need to calculate these using countValues function
    // average_mood: new Date().toDateString(),
    ...countValues(), // spreading them out from what countvalues is returning us
    time_remaining: `${23 - now.getHours()}H ${60 - now.getMinutes()}M`, // this too
  };
  ...
  async function handleSetMood(mood) {
    ...
    try {
      // take the mood and do the following
      ...
      const res = await setDoc(
        ...
        { merge: true }
      ); // we do not need to overwrite our backend data we just need to merge this into our existing one
    } catch (error) {
